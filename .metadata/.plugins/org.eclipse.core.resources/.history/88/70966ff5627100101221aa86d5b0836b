...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle, uint8_t SlaveAddr, uint8_t *pRxBuffer){
	//Interrupt handling for both master and slave mode of a device
		uint32_t temp1, temp2, temp3, dummy_read;
		/*Error Iterrupt enable and Event iterrupt enable*/
		temp1   = pI2CHandle->pI2Cx->CR2 & ( 1 << I2C_CR2_ITERR_EN) ;
		temp2   = pI2CHandle->pI2Cx->CR2 & ( 1 << I2C_CR2_ITBUF_EN) ;
		...
		if(temp1 && temp3){
			...
			if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX)
			{
				/*Send Slave Address*/
				 ...
				 pI2CHandle->DevAddr  &= ~(1); //SlaveAddr is Slave address + WRITE
				 pI2CHandle->pI2Cx->DR = pI2CHandle->DevAddr;
				 pI2CHandle->TxRxState = I2C_SEND_ADDR_W;

			}else if (pI2CHandle->TxRxState == I2C_BUSY_IN_RX )
			{
				pI2CHandle->DevAddr = pI2CHandle->DevAddr << 1;
				pI2CHandle->DevAddr  |= (1); //SlaveAddr is Slave address + READ
				pI2CHandle->pI2Cx->DR = pI2CHandle->DevAddr;
				pI2CHandle->TxRxState = I2C_SEND_ADDR_R;
		 }
		}
		...
		if(temp1 && temp3 && pI2CHandle->TxRxState == I2C_SEND_REG){

			/*Repetitive Start state*/
			/*Case of Transmission, Generate Stop condition*/
			if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX ){
				if(pI2CHandle->pI2Cx->SR1 & (1 << I2C_SR1_TXE)){
					if(pI2CHandle->TxLen == 0){
						if(pI2CHandle->Sr == I2C_STOP_ENABLE){
							pI2CHandle->pI2Cx->CR1 |= (1 << I2C_CR1_STOP);
						}
			...
			pI2CHandle->pI2Cx->CR1 |= (1 << I2C_CR1_START);
			pI2CHandle->TxRxState = I2C_SEND_REPEATED_START;
			/*Case of Reception*/
		}
				...
				if(pI2CHandle->TxRxState == I2C_BUSY_IN_RX){
					...
					if(pI2CHandle->RxLen > 1 ){
						...
						if(pI2CHandle->RxLen == 2){
							pI2CHandle->pI2Cx->CR1 |= (1<<I2C_ACK_DISABLE);
						}
						*(pI2CHandle->pRxBuffer) = pI2CHandle->pI2Cx->DR;
		...
		if(temp1 && temp2 && temp3 && pI2CHandle->TxRxState == I2C_RECEIVE_DATA){
			...
			if(pI2CHandle->pI2Cx->SR2 & ( 1 << I2C_SR2_MSL))
			{
				...
				*pI2CHandle->pRxBuffer = pI2CHandle->pI2Cx->DR;
				...
				if(pI2CHandle->RxLen == 13){
					// Disable ACK
					pI2CHandle->pI2Cx->CR1 &= ~(1 << I2C_CR1_ACK);
				}
				if(pI2CHandle->RxLen == 14){
					pI2CHandle->pI2Cx->CR1 |= (1 << I2C_CR1_STOP);
					pI2CHandle->TxRxState = I2C_STATE_IDLE;
					pI2CHandle->read_complete = 1;
				}
			}
		}
